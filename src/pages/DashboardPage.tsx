...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [
      ...
      (supabase as any)
        .from("schedules")
        .select("id, name, screen_id, enabled, start_time, end_time")
        .order("created_at"),
      (supabase as any)
        .from("media_items")
        .select("id, name, type")
      ...
      (supabase as any)
        .from("schedule_items")
        .select("id, schedule_id, media_id, duration")
        .order("sort_order"),
      (supabase as any)
  ...
  // Derived data
  const totalDevices = useMemo(() => screens.length, [screens]);

  const onlineCount = useMemo(
    () => screens.filter((s) => s.online === true).length,
    [screens]
  );

  const offlineCount = useMemo(
    () => screens.filter((s) => s.online === false).length,
    [screens]
  );

  const unknownCount = useMemo(
    () => screens.filter((s) => s.online !== true && s.online !== false).length,
    [screens]
  );
  ...
  const widgetCount = useMemo(
    () => mediaItems.filter((m) => m.type === "widget").length,
    [mediaItems]
  );

  const templateCount = useMemo(
    () => mediaItems.filter((m) => m.type === "template").length,
    [mediaItems]
  );
  ...
  const mediaTypeData = useMemo(() => {
    const data = [
      { name: t("image"), value: imageCount },
      { name: t("video"), value: videoCount },
      { name: t("audio"), value: audioCount },
      { name: t("widget"), value: widgetCount },
      { name: t("template"), value: templateCount },
    ].filter((d) => d.value > 0);

    return data;
  }, [imageCount, videoCount, audioCount, widgetCount, templateCount, t]);

  const mediaUsageData = useMemo(() => {
    const mediaTypeMap = new Map<string, string>();

    mediaItems.forEach((m) => {
      mediaTypeMap.set(m.id, m.type);
    });

    const typeUsageCount = {
      video: 0,
      image: 0,
      audio: 0,
      widget: 0,
    ...
    scheduleItems.forEach((si) => {
      if (!si.media_id) return;

      const mediaType = mediaTypeMap.get(si.media_id);

      if (mediaType === "video") typeUsageCount.video += 1;
      if (mediaType === "image") typeUsageCount.image += 1;
      if (mediaType === "audio") typeUsageCount.audio += 1;
      if (mediaType === "widget") typeUsageCount.widget += 1;
      if (mediaType === "template") typeUsageCount.template += 1;
    });

    return [
      { name: t("video"), count: typeUsageCount.video },
      { name: t("image"), count: typeUsageCount.image },
      { name: t("audio"), count: typeUsageCount.audio },
      { name: t("widget"), count: typeUsageCount.widget },
      { name: t("template"), count: typeUsageCount.template },
    ];
  }, [mediaItems, scheduleItems, t]);
  ...
  const scheduleOverview = useMemo(() => {
    ...
    return schedules.map((s) => {
      ...
      return {
        ...
        totalDuration,
        screenName: screen
          ? `${screen.branch || t("unknown")} – ${screen.name}`
          : "–",
      };
    });
  }, [schedules, scheduleItems, screens, t]);
    ...
    <div className="space-y-6 max-w-7xl">
      <div className="flex items-end justify-between animate-fade-in">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span className="text-muted-foreground/60">·</span>
          <span>
            {t("dashLastRefresh")} {format(lastRefresh, "HH:mm:ss")}
          </span>
        </div>
      </div>
      ...
      {emergencyCount > 0 && (
        <div className="rounded-xl border-2 border-destructive/40 bg-destructive/5 p-4 flex items-center gap-4 animate-pulse shadow-lg shadow-destructive/10">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              {t("dashEmergencyDesc").replace(
                "{count}",
                String(emergencyCount)
              )}
            </p>
          </div>
          <Button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
        ...
        <Card className="p-4 border-success/20 hover-lift shadow-sm opacity-0 animate-count-up stagger-1">
          <div className="flex items-center justify-between mb-2">
            ...
            <Badge
              variant="outline"
              className="text-[10px] text-success border-success/30 bg-success/5"
            >
              LIVE
            </Badge>
          </div>
        ...
        {/* Unknown */}
        <Card className="p-4 border-muted/30 hover-lift shadow-sm opacity-0 animate-count-up stagger-4">
          <div className="flex items-center justify-between mb-2">
            <div
              className="w-9 h-9 rounded-lg bg-muted/20 flex items-center justify-center"
              title={t("unknown")}
            >
            ...
            {unknownCount > 0 && (
              <Badge
                variant="outline"
                className="text-[10px] border-muted-foreground/20 bg-muted/40 text-muted-foreground"
              >
                ?
              </Badge>
            )}
          ...
          <p className="text-xs text-muted-foreground mt-0.5">{t("unknown")}</p>
        </Card>
      ...
      <div className="animate-fade-in" style={{ animationDelay: "0.45s" }}>
        ...
        {screens.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {screens.map((screen, i) => {
              const isOnline = screen.online === true;
              const isOffline = screen.online === false;
              const isUnknown = screen.online !== true && screen.online !== false;
                  ...
                  <div className="aspect-video bg-muted relative flex items-center justify-center overflow-hidden">
                    ...
                    <div
                      className={cn(
                        ...
                        isUnknown && "bg-muted/70 text-muted-foreground"
                      )}
                    >
                      <span
                        className={cn(

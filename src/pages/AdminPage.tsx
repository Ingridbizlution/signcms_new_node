...
export default function AdminPage() {
  ...
  const handleRoleChange = async () => {
    ...
    if (error) {
      toast.error("角色更新失敗：" + error.message);
    } else {
      toast.success(`已將 ${user.display_name || "使用者"} 的角色更新為${newRole === "admin" ? "管理員" : "一般使用者"}`);
      fetchUsers();
    }
    setSaving(false);
      ...
      <div className="flex items-center justify-center min-h-[400px]">
        <Card className="max-w-md w-full">
          <CardContent className="pt-6 text-center space-y-3">
            ...
            <h2 className="text-lg font-semibold text-foreground">權限不足</h2>
            <p className="text-sm text-muted-foreground">
              您沒有管理員權限，無法存取此頁面。請聯繫系統管理員。
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="animate-fade-in">
        ...
        <p className="text-sm text-muted-foreground mt-1">管理系統使用者角色與權限</p>
      </div>
        ...
        <Card className="opacity-0 animate-fade-in stagger-1">
          <CardContent className="pt-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">總使用者數</p>
            </div>
          </CardContent>
        </Card>
        <Card className="opacity-0 animate-fade-in stagger-2">
          <CardContent className="pt-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">管理員數</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="opacity-0 animate-fade-in stagger-3">
        <CardHeader>
          ...
          <CardDescription>查看和管理所有使用者的角色</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Select
                      ...
                      onValueChange={(value: "admin" | "user") => {
                        if (value !== user.role) {
                          setChangeDialog({ user, newRole: value });
                        }
                      }}
                    >
      ...
      <Dialog open={!!changeDialog} onOpenChange={() => setChangeDialog(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>確認更改角色</DialogTitle>
            <DialogDescription>
              確定要將 <strong>{changeDialog?.user.display_name || "此使用者"}</strong> 的角色更改為
              <strong>{changeDialog?.newRole === "admin" ? " 管理員" : " 一般使用者"}</strong>？
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setChangeDialog(null)}>
              取消
            </Button>

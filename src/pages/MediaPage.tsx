...
function parseWidgetConfig(url: string): WidgetConfig | null {
  if (!url?.startsWith("widget://")) return null;
  try {
    return JSON.parse(url.replace("widget://", ""));
  } catch {
    return null;
  }
}
...
function WidgetPreviewCard({ config }: { config: WidgetConfig }) {
      ...
      return (
        <div className="w-full h-full flex items-center justify-center" style={{ background: bg }}>
          <svg viewBox="0 0 200 200" className="w-[70%] max-w-[160px]">
            ...
            <polygon
              points={`${100 + 45 * Math.cos(((hDeg - 90) * Math.PI) / 180)},${100 + 45 * Math.sin(((hDeg - 90) * Math.PI) / 180)} ${100 + 5 * Math.cos((hDeg * Math.PI) / 180)},${100 + 5 * Math.sin((hDeg * Math.PI) / 180)} ${100 - 10 * Math.cos(((hDeg - 90) * Math.PI) / 180)},${100 - 10 * Math.sin(((hDeg - 90) * Math.PI) / 180)} ${100 - 5 * Math.cos((hDeg * Math.PI) / 180)},${100 - 5 * Math.sin((hDeg * Math.PI) / 180)}`}
              fill={fg}
              opacity="0.9"
            />
            <polygon
              points={`${100 + 65 * Math.cos(((mDeg - 90) * Math.PI) / 180)},${100 + 65 * Math.sin(((mDeg - 90) * Math.PI) / 180)} ${100 + 4 * Math.cos((mDeg * Math.PI) / 180)},${100 + 4 * Math.sin((mDeg * Math.PI) / 180)} ${100 - 12 * Math.cos(((mDeg - 90) * Math.PI) / 180)},${100 - 12 * Math.sin(((mDeg - 90) * Math.PI) / 180)} ${100 - 4 * Math.cos((mDeg * Math.PI) / 180)},${100 - 4 * Math.sin((mDeg * Math.PI) / 180)}`}
              fill={fg}
              opacity="0.85"
            />
            <line
              x1={100 - 18 * Math.cos(((sDeg - 90) * Math.PI) / 180)}
              y1={100 - 18 * Math.sin(((sDeg - 90) * Math.PI) / 180)}
              x2={100 + 72 * Math.cos(((sDeg - 90) * Math.PI) / 180)}
              y2={100 + 72 * Math.sin(((sDeg - 90) * Math.PI) / 180)}
              stroke="hsl(0 70% 55%)"
              strokeWidth="1.2"
              strokeLinecap="round"
            />
  ...
  if (config.widgetType === "marquee" && config.text) {
    return (
      <div className="w-full h-full flex items-center overflow-hidden" style={{ background: bg, color: fg }}>
        <div className={`animate-marquee whitespace-nowrap ${zfs.marquee} font-medium`}>
          {config.text}
        </div>
      </div>
    );
  ...
  if (config.widgetType === "webpage") {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center gap-1" style={{ background: bg, color: fg }}>
        <Globe className="w-6 h-6 opacity-50" />
        <span className="text-[10px] opacity-60 truncate max-w-[80%]">
          {config.url || "URL"}
        </span>
      </div>
    );
    ...
    return (
      <div className="w-full h-full flex flex-col items-center justify-center gap-1" style={{ background: bg, color: fg }}>
        {config.countdownTitle && (
          <span className={`${zfs.title} font-bold opacity-70`}>
            {config.countdownTitle}
          </span>
        )}
        <div className="flex gap-2">
          {[days, hours, mins, secs].map((v, i) => (
            <span key={i} className={`${zfs.countdown} font-mono font-bold`}>
...
const getTypeBadgeVariant = (type: MediaType) =>
  type === "widget" ? "default" : "secondary";
...
const MediaPage = () => {
  ...
  const fetchMedia = async () => {
    setLoading(true);
    setMedia([...mockMedia].sort((a, b) => b.created_at.localeCompare(a.created_at)));
    setLoading(false);
  };

  const fetchProjects = async () => {
    setProjects([...mockProjects].sort((a, b) => a.name.localeCompare(b.name)));
  };
  ...
  const filteredMedia = useMemo(() => {
    return media.filter((item) => {
      ...
      const matchesProject =
        projectFilter === "all" ||
        (projectFilter === "none"
          ? !item.design_project_id
          : item.design_project_id === projectFilter);
    ...
    try {
      ...
      if (isVideo) {
        ...
        const videoMeta = await new Promise<{ dimensions: string; duration: string | null }>((resolve) => {
          ...
          video.onerror = () => {
            resolve({ dimensions: "-", duration: null });
            URL.revokeObjectURL(objectUrl);
          };
          video.src = objectUrl;
        });
      ...
      const newItem: MediaItemRow = {
        ...
        url: tempUrl,
        thumbnail: isImage ? tempUrl : null,
  ...
  const checkMediaUsage = async (mediaId: string) => {
    ...
    setDeleteUsage({ schedules: [], projects: projectNames });
    setCheckingUsage(false);
  };
  ...
  const handleDelete = async () => {
    ...
    const item = media.find((entry) => entry.id === deleteId);
    setMedia((current) => current.filter((entry) => entry.id !== deleteId));
    toast.success(`${t("mediaDeleted")}：${item?.name || ""}`);
    logActivity({
      action: "刪除素材",
      category: "media",
  ...
  const handleChangeProject = async (itemId: string, newProjectId: string | null) => {
    ...
    setMedia((current) =>
      current.map((item) =>
        item.id === itemId ? { ...item, design_project_id: newProjectId } : item
      )
    );
    setPreviewItem((current) =>
      current && current.id === itemId
        ? { ...current, design_project_id: newProjectId }
        : current
    );
    toast.success(t("save"));
    setUpdatingProjectItemId(null);
  ...
  const handleCreateProject = async () => {
    ...
    setProjects((current) => [...current, newProject].sort((a, b) => a.name.localeCompare(b.name)));
    setNewProjectName("");
    toast.success(t("mediaProjectCreated"));
  };
  ...
  const handleUpdateProject = async () => {
    ...
    setProjects((current) =>
      current
        .map((project) =>
          project.id === editingProject.id ? { ...project, name } : project
        )
  ...
  const handleDeleteProject = async () => {
    ...
    setMedia((current) =>
      current.map((item) =>
        item.design_project_id === deleteProjectId
          ? { ...item, design_project_id: null }
          : item
      )
    );

    setProjects((current) => current.filter((project) => project.id !== deleteProjectId));
    toast.success(t("mediaProjectDeleted"));
    setDeleteProjectId(null);
  };
  ...
  const handleCreateWidget = async () => {
    if (!widgetForm.name.trim()) {
      toast.error(t("widgetFillRequired"));
      return;
    }
    ...
    const newWidget: MediaItemRow = {
      ...
      created_at: new Date().toISOString(),
      design_project_id:
        widgetForm.projectId !== NONE_PROJECT_VALUE
          ? widgetForm.projectId
          : null,
      is_system: false,
    };

    setMedia((current) => [newWidget, ...current]);
    toast.success(t("widgetCreated"));
    setWidgetDialogOpen(false);
    setWidgetForm({ ...defaultWidgetForm });
  };
    ...
    return (
      <div onClick={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}>
        <Select
          value={item.design_project_id || NONE_PROJECT_VALUE}
          onValueChange={(value) =>
            handleChangeProject(item.id, value === NONE_PROJECT_VALUE ? null : value)
          }
          disabled={updatingProjectItemId === item.id}
        >
          <SelectTrigger
            className={`border-0 bg-transparent px-0 py-0 shadow-none focus:ring-0 focus:ring-offset-0 ${compact
                ? "h-6 gap-1 text-xs text-muted-foreground hover:text-foreground"
                : "h-6 gap-1 text-[11px] text-muted-foreground hover:text-foreground"
              }`}
          >
            <FolderOpen className="w-3 h-3 shrink-0" />
            <SelectValue placeholder={t("mediaNoProject")} />
          ...
          {filteredMedia.map((item) => (
            <Card key={item.id} className="overflow-hidden cursor-pointer" onClick={() => setPreviewItem(item)}>
              <div className="aspect-video bg-muted flex items-center justify-center overflow-hidden relative">
                {item.type === "widget" ? (
                  (() => {
                    const c = parseWidgetConfig(item.url);
                    return c ? (
                      <WidgetPreviewCard config={c} />
                    ) : (
                ...
                {item.type === "widget" && (
                  <Badge
                    variant={item.is_system ? "destructive" : "outline"}
            ...
            <Card key={item.id} className="flex cursor-pointer items-center gap-4 p-3" onClick={() => setPreviewItem(item)}>
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                  ...
                  {item.type === "widget" && (
                    <Badge
                      variant={item.is_system ? "destructive" : "secondary"}
              ...
              {checkingUsage ? (
                ...
                <div className="space-y-2">
                  ...
                  {deleteUsage.projects.length > 0 && (
                    <div>
                      <p className="text-xs text-muted-foreground">
                        {t("mediaUsedInProjects")}：
                      </p>
                      <ul className="list-disc list-inside text-sm">
                        {deleteUsage.projects.map((n) => (
                          <li key={n}>{n}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {deleteUsage.schedules.length > 0 && (
                    <div>
                      <p className="text-xs text-muted-foreground">
                        {t("mediaUsedInSchedules")}：
                      </p>
                      <ul className="list-disc list-inside text-sm">
                        {deleteUsage.schedules.map((n) => (
                          <li key={n}>{n}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {editingProject?.id === project.id ? (
                    <div className="flex flex-1 gap-2">
                      <Input
                        ...
                        autoFocus
                      />
                      <Button size="sm" className="h-8" onClick={handleUpdateProject} disabled={!editProjectName.trim()}>
                        {t("save")}
                      </Button>
                      <Button
                        size="sm"
        ...
        <DialogContent className="sm:max-w-lg max-h-[85vh] flex flex-col">
          ...
          <div className="space-y-4 py-2 overflow-y-auto flex-1 pr-1 min-h-0">
            ...
            <div className="space-y-2">
              <Label>{t("mediaProjectGroup")}</Label>
              <Select
                value={widgetForm.projectId}
                onValueChange={(v) => setWidgetForm({ ...widgetForm, projectId: v })}
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  <SelectItem value={NONE_PROJECT_VALUE}>
                    {t("mediaNoProject")}
                  </SelectItem>
                  {projects.map((p) => (
              ...
              <div className="grid grid-cols-4 gap-2">
                {(
                  ...
                  const labels: Record<WidgetSubType, string> = {
                    ...
                    qrcode: t("widgetQrcode"),
                    countdown: t("widgetCountdown"),
                    youtube: t("widgetYoutube"),
                  ...
                  return (
                    <button
                      key={wt}
                      type="button"
                      onClick={() => setWidgetForm({ ...widgetForm, widgetType: wt })}
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-lg border-2 transition-all text-center ${widgetForm.widgetType === wt
                          ? "border-primary bg-primary/5"
                          : "border-border hover:border-primary/40"
                        }`}
                    >
                      <Icon
                        className={`w-6 h-6 ${widgetForm.widgetType === wt
                            ? "text-primary"
              ...
              <div className="space-y-3">
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      type="button"
                      onClick={() => setWidgetForm({ ...widgetForm, clockStyle: "digital" })}
                      className={`p-2.5 rounded-lg border-2 text-sm ${widgetForm.clockStyle === "digital"
                    ...
                    <button
                      type="button"
                      onClick={() => setWidgetForm({ ...widgetForm, clockStyle: "analog" })}
                      className={`p-2.5 rounded-lg border-2 text-sm ${widgetForm.clockStyle === "analog"
                          ? "border-primary bg-primary/5 text-primary font-medium"
                          : "border-border hover:border-primary/40"
                        }`}
                    >
                ...
                {widgetForm.clockStyle === "digital" && (
                  <div className="space-y-2">
                    <Label>{t("widgetFormat")}</Label>
                    <Select
                      value={widgetForm.format}
                      onValueChange={(v) =>
                        setWidgetForm({ ...widgetForm, format: v as "12" | "24" })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                ...
                <div className="flex items-center justify-between">
                  <Label>{t("widgetShowDate")}</Label>
                  <Switch
                    checked={widgetForm.showDate}
                    onCheckedChange={(v) => setWidgetForm({ ...widgetForm, showDate: v })}
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label>{t("widgetTimezone")}</Label>
                  <Select
                    value={widgetForm.timezone}
                    onValueChange={(v) => setWidgetForm({ ...widgetForm, timezone: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
            ...
            {widgetForm.widgetType === "webpage" && (
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setWidgetForm({ ...widgetForm, url: e.target.value })}
            ...
            {widgetForm.widgetType === "marquee" && (
              <>
                <div className="space-y-2">
                  ...
                  <Input
                    value={widgetForm.text}
                    onChange={(e) => setWidgetForm({ ...widgetForm, text: e.target.value })}
                ...
                <div className="space-y-2">
                  <Label>{t("widgetSpeed")}</Label>
                  <Select
                    value={widgetForm.speed}
                    onValueChange={(v) =>
                      setWidgetForm({
                        ...widgetForm,
                        speed: v as "slow" | "normal" | "fast",
                      })
                    }
                  >
                    <SelectTrigger>
            ...
            {widgetForm.widgetType === "qrcode" && (
              <div className="space-y-2">
                ...
                <Input
                  value={widgetForm.qrcodeContent}
                  onChange={(e) =>
                    setWidgetForm({ ...widgetForm, qrcodeContent: e.target.value })
                  }
                  placeholder={t("widgetQrcodePlaceholder")}
                />
              </div>
            )}
            ...
            {widgetForm.widgetType === "countdown" && (
              <>
                <div className="space-y-2">
                  ...
                  <Input
                    value={widgetForm.countdownTitle}
                    onChange={(e) =>
                      setWidgetForm({ ...widgetForm, countdownTitle: e.target.value })
                    }
                    placeholder={t("widgetCountdownTitlePlaceholder")}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    type="datetime-local"
                    value={widgetForm.targetDate}
                    onChange={(e) =>
                      setWidgetForm({ ...widgetForm, targetDate: e.target.value })
                    }
                  />
                </div>
              </>
            ...
            {widgetForm.widgetType === "youtube" && (
              <div className="space-y-2">
                ...
                <Input
                  value={widgetForm.youtubeUrl}
                  onChange={(e) =>
                    setWidgetForm({ ...widgetForm, youtubeUrl: e.target.value })
                  }
                  placeholder={t("widgetYoutubeUrlPlaceholder")}
                />
              </div>
            )}
            ...
            {widgetForm.widgetType === "weather" && (
              <div className="space-y-2">
                ...
                <Input
                  value={widgetForm.city}
                  onChange={(e) => setWidgetForm({ ...widgetForm, city: e.target.value })}
                  placeholder={t("widgetCityPlaceholder")}
                />
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="color"
                    value={widgetForm.textColor}
                    onChange={(e) =>
                      setWidgetForm({ ...widgetForm, textColor: e.target.value })
                    }
                    className="w-8 h-8 rounded border border-border cursor-pointer"
                  />
                  <Input
...
function PreviewInfoPanel({
  ...
  const fields = [
    ...
    { label: t("mediaEncoding"), value: encoding },
    ...(item.type === "image"
      ? [{ label: t("mediaPixels"), value: extraInfo.pixels || "..." }]
      : []),
    ...(item.type === "video"
      ? [
        { label: t("mediaDuration"), value: item.duration || "-" },
    ...
    <div className="rounded-lg border border-border bg-muted/30 p-3 text-xs">
      <table className="w-full">
        <tbody>
          {fields.map((f) => (
            <tr key={f.label} className="border-b border-border/40 last:border-0">
              <td className="py-1 pr-3 text-muted-foreground whitespace-nowrap w-[1%]">
                {f.label}
              </td>
              <td className="py-1 font-medium text-foreground break-all">
                {f.value}
              </td>
            </tr>
          ))}

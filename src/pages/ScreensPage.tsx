...
import { supabase } from "@/integrations/supabase/client";
import { mockScreens } from "@/mock/screensMockData";
import { mockMedia, mockProjects } from "@/mock/mediaMockData";
import { Card } from "@/components/ui/card";
import {
  Table, TableBody, TableCell, TableHead, TableHeader, TableRow,
} from "@/components/ui/table";
...
export default function ScreensPage() {
  ...
  useEffect(() => {
    ...
    const designFromMock = mockProjects
      .map((project) => ({
        id: project.id,
        name: project.name,
      }))
    ...
    setMediaOptions(mediaFromMock);
    setDesignOptions(designFromMock);
  }, [settingsScreen]);
      ...
      <div className="flex flex-wrap gap-2">
        {(
          ...
          <button
            key={key}
            onClick={() => setStatusFilter(statusFilter === key ? "all" : key)}
            className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium transition-all ${statusFilter === key
                ? "border-primary bg-primary/10 text-primary"
                : "border-border bg-card text-muted-foreground hover:bg-accent"
              }`}
          >
          ...
          <Table>
            ...
            <TableBody>
              {filtered.map((screen) => (
                <TableRow key={screen.id} className="hover:bg-muted/30">
                  <TableCell>
                    <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium ${screen.online ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"
                      }`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${screen.online ? "bg-success" : "bg-muted-foreground"}`} />
                      {screen.online ? t("online") : t("offline")}
                    </span>
                    ...
                    {iotDevices.map((device) => {
                      ...
                      return (
                        <div key={device.id} className={`flex items-center gap-3 p-3 rounded-lg border ${cfg.color}`}>
                          ...
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium ${device.status === "online" ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"
                            }`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${device.status === "online" ? "bg-success" : "bg-destructive"}`} />
                            {device.status === "online" ? "連線中" : "離線"}
                          </span>
